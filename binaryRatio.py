...
class ratio(Dataset): # start with binary classification? 
    def __init__(self, original_dataset, num_classes, target_ratios): # target_ratios is a list   
        ...
        class_images = []
        class_images.append(images[(labels == 0)].unsqueeze(1)) # same shape as labels w/ both classes
        class_images.append(images[(labels == 1)].unsqueeze(1))


        class_labels = []
        class_labels.append(labels[(labels == 0)]) 
        class_labels.append(labels[(labels == 1)])
